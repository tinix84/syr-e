...
%% Matteo mod per rendere la geo simile a Vdes, mod provvisoria: 
% s.DS1=acs*PassoCava/2;                % semi-apertura cava           
% s.DS2=s.DS1*0.563;                    % provvisorio per far assomigliare lo statore ad Electroadda S1
% s.RS3=s.RSI+s.DS2;
...
% corretto 24 08 09
s.RS1 = s.RS3+s.DS1;            %tut. pag.3.31 (d0/c0=b/2)
...
%% Possibile soluzione per raccordo di fondo cava
RaccordoFC=abs((s.RS2-s.RS5)+1j*(s.RC2-s.DS3));
s.RS6=(s.RS5-RaccordoFC)*cos(s.AS1/2*pi/180);
s.RD6=s.RS6*cos(s.AS1*pi/180); s.RC6=s.RC1+(s.RS6-s.RS1)*sin(s.AS1*pi/180);
% old
