...
% keyboard
%% Set stator portion over evaluate iron losses
% Ncoils=3;
Porz_stat=Mac.Qs;
Nbarrier=0;
%%
...
for ii=1:NTime
    ...
    for jjj = 1:n_mag
        if strcmp(geo.RotType,'SPM')
            Command = ['PowerLoss=getDocument().getSolution().getOhmicLossInConductor(ProblemID,"PM_' num2str(jjj) '")'];
        else
            Command = ['PowerLoss=getDocument().getSolution().getOhmicLossInConductor(ProblemID,"Plasto_Bar_' num2str(jjj) '")'];
        end
        invoke(MN6, 'processCommand', Command);
