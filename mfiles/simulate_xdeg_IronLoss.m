...
function [SOL,FluxDens] = simulate_xdeg(geo,io,gamma_in,eval_type)
...
for jj = 1:nsim
    th_m = (th(jj) - th0)/p;
    if th_m < (360/(p*2*ps))-((360/(p*2*ps))/Qs/2)                  % MAURIZIO Fe Loss                                                       %%%
        openfemm
    ...
    else              %% MAU --> Serve per permettere la simulazione di un semiperiodo elettrico
        
        th_m = ((th(jj) - th0)/p) - (360/p*ps);
        openfemm
        opendocument([pathname,'\mot0.fem']);
