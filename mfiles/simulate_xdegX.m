...
function [SOL,FemmProblem] = simulate_xdegX(FemmProblem,geo,io,gamma_in,eval_type)
...
for jj = 1:nsim
    ...
    % redraw the airgap arc
    FemmProblem=draw_airgap_arc_with_meshX(FemmProblem,geo,th_m,geo.fem,geo.boundnameAPmove);
    
    filename = ['mot' num2str(0) 'X.fem'];
    writefemmfile(filename, FemmProblem);
     save('mot0X','FemmProblem')
