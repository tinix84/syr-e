...
% Initial file: Marco Gallo (Master thesis), July 2018
% Modification: Simone Ferrari (NC State project), July-December 2018
...
% % SF: normalizzazione più corretta - scommentare entro fine ottobbre,
% % oppure eliminare
hc_half_min = pont0;
hc_half_max = la/4;
...
b=sqrt((XMagpontRadSx(1)-XMagpontRadDx(1))^2+(YMagpontRadSx(1)-YMagpontRadDx(1))^2); %base area magnete 
l=sqrt((XMag6(1)-XMagpontRadDx(1))^2+(YMag6(1)-YMagpontRadDx(1))); %altezza area magnete (da lato esterno);
...
xair=(xpont+XMag5+XMag6)/3; %definizione punto medio porzione estrema barriera (parte raccordata superiore verso ponticello tangenziale)
yair=(ypont+YMag5+YMag6)/3;
if pont>0
...
elseif ((angle<=angle_FEMM)&&(angle~=90)) % magnete rettangolare, devo aggiungere porzione di aria
    xtemp=(B1k+B2k+XMagpontRadSx)/3; 
    ytemp=(0+0+YMagpontRadSx)/3;
    xair=[xair,xtemp];
    yair=[yair,ytemp];
end
