...
function evalParetoFrontX(filename)
...
parfor m = 1:size(x,1)
    ...
    [cost{m},geometry{m},~,~] = FEMMfitnessX(x(m,:)',geo,per_temp,eval_type);
...
for m=1:size(x,1)
    ...
    if exist([syreRoot filesep 'empty_case.fem'],'file')>1
        FemmProblem = loadfemmfile([syreRoot filesep 'empty_case.fem']);
    else
        FemmProblem = loadfemmfile(['c:' filesep 'empty_case.fem']);
    end
    ...
    % Save png file mot
    H=plotfemmproblem(FemmProblem);
    print(H,[pathname '\mot_'  mot_index '.png'],'-dpng');
    % Save .fem file mot
    writefemmfile('mot0X.fem', FemmProblem);
    movefile([syreRoot,'\mot0X.fem'],[pathname '\mot_'  mot_index '.fem']);
    % Save data geometry mot
	geo.RQ = x(m,:);
    save([pathname '\mot_' mot_index '.mat'],'geo','per');
    %% %%%%%%%
    COST=[COST; cost{m}];
    if m<10
        mot_index = ['0' mot_index];
