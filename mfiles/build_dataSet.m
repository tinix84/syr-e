...
dataSet.PitchShortFac = round(geo.kracc,2);                 % pitch short factor
...
dataSet.Mesh = geo.K_mesh;                                  % for post-processing and manual design
dataSet.Mesh_MOOA = geo.K_mesh_MOOA;                        % for optimization

%% MOOA setting

dataSet.SimPoMOOA = geo.nsim_MOOA;                          % number of rotor position
dataSet.RotPoMOOA = geo.delta_sim_MOOA;                     % rotor position span [elt degrees]
dataSet.SimPoFine = geo.nsim_singt;                         % simulated position in the re-evaluation of Pareto front
dataSet.RotPoFine = geo.delta_sim_singt;                    % rotor position span in the re-evaluation of Pareto front [elt degrees]
